...
const Home = ({ navigation }) => {
    ...
    if (user) {
      (async () => {
        try {
          ...
          const usuario = { name: user.name, email: user.email, id:user.nickname };
          await saveToken(creds.accessToken)
          navigation.replace('Bienvenido', { usuario });
          console.log(usuario)
        } catch (e) {
  ...
  const onLogin = async () => {
    try {
      ...
      setCredentials(creds);
      /* console.log(creds) */
      const usuario = { name: user.name, email: user.email, id:user.nickname, accesToken:creds.accessToken };
